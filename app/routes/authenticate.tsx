...
import { logger } from "cross-kit/lib/logger";
import { auth } from "~/lib/auth.server";
import type { ApiResponse } from "@/cross-kit/utility/types/api-types";
import {
  catchError,
  catchTypedError,
} from "@/cross-kit/utility/functions/error-handler";
import {
  validateEMail,
  validatePassword,
} from "~/utility/functions/validation";
import { useEffect, useState } from "react";
import { toast } from "sonner";
import { APIError } from "better-auth/api";

const homePage = "/app/dashboard";
...
export async function action({ request }: ActionFunctionArgs) {
  if (request.method !== "POST") {
    logger.warn("Authentication attempted with invalid method", {
  ...
  try {
    ...
    logger.info("Authentication attempt", { email: email, type: type });
    ...
    if (type === "signin") {
      ...
      logger.info("Sign In Successful", userData.response.url);
      return data<ApiResponse>(
        {
          success: true,
    ...
    logger.error("Failed attempt to authenticate", error);
    return data<ApiResponse>(
      {
        success: false,
...
export default function Authenticate() {
  ...
  return (
    <div className="transition-all bg-muted flex min-h-svh flex-col items-center justify-center p-6 md:p-10">
          ...
          <Card className="overflow-hidden p-0">
            <CardContent className="grid p-0 md:grid-cols-2">
              ...
              <div className="bg-muted relative hidden md:block">
                <img
                  src="/assets/images/banners/toned-shore.webp"
                  alt="Poster Image"
                  className="absolute inset-0 h-full w-full object-cover dark:brightness-[0.2] dark:grayscale bg-background"
                />
              </div>
            </CardContent>
          </Card>
